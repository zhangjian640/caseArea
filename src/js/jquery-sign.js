  ...
  function defined(dom) {
    $(document).on("mousedown", dom, function (e) {
      ...
      if (e.which == 3) {
        ...
        var t = e.clientY + $(document).scrollTop() - $(dom).offset().top;
    ...
    $(document).on("click", "#addsign", function (e) {
      ...
      $('.inputSignBox').append("<div class='outSignbox'>关闭</div>");
    ...
    $(document).on('click', '.sureSign', function () {
      ...
      //   var text = $('.signbox').text();
        $('.inputSignBox').remove();
        $(dom).append("<div class='signIndex' id='Ts" + indexId  + "'></div>");
        $('#Ts' + indexId).css({"left": cX - 15, "top": cY - 15});
        // if (changeSignColor) {
        //   $('#Ts' + indexId).css("border", signColor + " 3px solid");
        // }//改变了颜色
        indexId++;
        // var mes = {left: cX - 15, top: cY - 15, message: tex};
    ...
    $(document).on('mousedown', '[id*=Ts]', function (e) {
      ...
      if (e.which == 3) {
        ...
        $('.chooseBox').css({"left": l, "top": t + $(document).scrollTop()});
      }
    });//弹出取消标记
